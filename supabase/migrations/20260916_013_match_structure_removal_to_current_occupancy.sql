-- Match removal protection to the occupancy displayed by the application:
-- only active assignments of the corresponding rental-space type block removal.
-- Active assignments keep strict location requirements. Historical assignments
-- may have deleted room/bed references cleared without deleting their records.
alter table public.tenant_assignments
  drop constraint if exists tenant_assignments_structure_chk;

alter table public.tenant_assignments
  add constraint tenant_assignments_structure_chk check (
    (
      status = 'active'
      and (
    ...
    or (
      status <> 'active'
      and (
        assignment_type = 'bed_space'
        or (assignment_type = 'room_space' and bed_space_id is null)
        or (assignment_type = 'apartment' and room_id is null and bed_space_id is null)
      )
    )
  );
...
begin
  ...
  where r.unit_id = p_unit_id
    ...
    and not exists (
      ...
      where ta.bed_space_id = bs.id
        and ta.assignment_type = 'bed_space'
  ...
  where r.unit_id = p_unit_id
    ...
    and not exists (
      ...
      where ta.room_id = r.id
        and ta.assignment_type = 'room_space'
        and ta.status = 'active'
    )
    and not exists (
      select 1
  ...
  set room_id = null,
      bed_space_id = null
  where ta.room_id = any(removable_room_ids)
     or exists (
       select 1
       from public.bed_spaces bs
       where bs.room_id = any(removable_room_ids)
