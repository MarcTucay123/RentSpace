-- Maintenance history no longer prevents room or bed-space removal. Preserve
-- each maintenance request while clearing only references to deleted structure.
-- Tenant assignment history remains the sole deletion protection.
...
begin
  ...
  where r.unit_id = p_unit_id
    ...
    and not exists (
      select 1 from public.tenant_assignments ta where ta.bed_space_id = bs.id
    );
  ...
  where r.unit_id = p_unit_id
    ...
    and not exists (
      select 1 from public.tenant_assignments ta where ta.room_id = r.id
    )
    and not exists (
      select 1
      from public.bed_spaces bs
