...
## 1. `users`
- **Purpose:** Main application user record linked 1-to-1 with Supabase Auth's separate `auth.users` table.
- **Relationships:** `public.users.id -> auth.users.id`.
...
- **Relationships:** `properties.landlord_id -> users.id`.
...
- **Relationships:** `tenant_profiles.profile_id -> users.id` (unique), optional `property_id -> properties.id`.
...
- **Relationships:** `payment_proofs.payment_id -> payments.id`, `uploaded_by -> users.id`.
...
- **Relationships:** `recipient_profile_id -> users.id`.
...
- Trigger to auto-create a default `public.users` row for new Auth users as `landlord/pending` or `tenant/pending` based on controlled registration metadata.
