...
export function MessageInbox({ currentProfileId, basePath, contacts, messages, selectedContactId, initialDraft = "", searchEnabled = true }: MessageInboxProps) {
  ...
  return (
    <div className="grid h-[min(680px,calc(100dvh-2rem))] min-h-[520px] grid-rows-[auto_minmax(0,1fr)] overflow-hidden rounded-[1.35rem] border border-[var(--color-dormmate-border)] bg-white shadow-[var(--shadow)] lg:h-[min(680px,calc(100dvh-7rem))] lg:grid-cols-[280px_minmax(0,1fr)] lg:grid-rows-1">
      <aside className="flex min-h-0 flex-col border-b border-[var(--color-dormmate-border)] bg-[#f5f6f4] p-3 lg:border-b-0 lg:border-r lg:p-4">
        ...
        <nav aria-label="Message contacts" className="flex gap-2 overflow-x-auto pb-1 lg:grid lg:min-h-0 lg:flex-1 lg:content-start lg:overflow-x-hidden lg:overflow-y-auto lg:overscroll-contain lg:pr-1">
      ...
      <section className="flex min-h-0 min-w-0 flex-col">
        {selectedContact ? (
          <>
            <header className="flex items-center justify-between gap-3 border-b border-[var(--color-dormmate-border)] px-4 py-3 sm:px-5">
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  {selectedContact.role === "tenant" ? <p className="text-xs font-medium text-[var(--color-dormmate-primary)]">{selectedContact.assignmentLabel ?? "No current assignment"}</p> : <p className="text-xs capitalize text-[var(--color-dormmate-muted)]">{selectedContact.role}</p>}
                </div>
              </div>
              {unreadSelected > 0 ? (
                <button
            ...
            <div className="min-h-0 flex-1 space-y-3 overflow-y-auto overscroll-contain bg-[#f8f8f6] p-4 sm:p-5">
              {selectedMessages.length === 0 ? (
                ...
                return (
                  <article key={message.id} className={`flex ${outgoing ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[85%] rounded-[18px] px-4 py-3 sm:max-w-[70%] ${outgoing ? "rounded-br-md bg-[#1b263b] text-white" : "rounded-bl-md border border-[var(--color-dormmate-border)] bg-white text-[#0d1b2a]"}`}>
            ...
            <form action={action} className="border-t border-[var(--color-dormmate-border)] p-3 sm:p-4">
              ...
              <div className="flex items-end gap-2">
                <textarea name="body" required maxLength={2000} rows={2} defaultValue={initialDraft} placeholder="Write a message..." className="min-h-[48px] flex-1 resize-none rounded-[16px] border border-[var(--color-dormmate-border)] bg-[#f8f8f6] px-4 py-3 text-sm outline-none focus:border-[var(--color-dormmate-primary)] focus:ring-2 focus:ring-[var(--color-dormmate-primary-soft)]" />
                <button type="submit" disabled={pending} className="rounded-[14px] bg-[var(--color-dormmate-primary)] px-4 py-3 text-sm font-semibold text-white transition hover:opacity-90 disabled:opacity-60">
